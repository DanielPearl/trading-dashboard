"""NBA bot — in-process trading loop for KXNBAGAME game markets.
Shape B (hand-rolled tick), structurally identical to the MLB bot.

2026-07 rearchitecture: previously Shape A — the upstream nba_bot
package's Bot class (Elo model + ESPN injuries) ran its own loop into
...
(devigged Pinnacle NBA moneyline via kalshi_sdk.pinnacle — guest feed
...
Elo path is retired from trading. (That also retires the
stats.nba.com rate-limit workarounds the old wrapper carried — the
benchmark path never touches nba_api.) Rows without a benchmark line
are never buy-eligible, and out of season (no open KXNBAGAME markets)
...
def _load_upstream(repo_path: str) -> dict[str, Callable[..., Any]]:
    """Load the NBA upstream under a unique alias to avoid the
    ``src/`` namespace collision with the other sport bots."""
    import importlib
    _base.load_upstream_as_alias(repo_path, "nba_src", subdir="src")

    kalshi_markets = importlib.import_module(
        "nba_src.data.kalshi_markets",
    )
    export_watchlist_mod = importlib.import_module(
        "nba_src.dashboard.export_watchlist",
    )
    simulator_mod = importlib.import_module(
        "nba_src.trading.simulator",
    )

    return {
        "fetch_markets": kalshi_markets.fetch_nba_markets,
...
def _one_tick(upstream: dict[str, Callable[..., Any]],
    ...
    log.info(
        "nba tick — %d markets / %d games / %d rows / "
...
def start_daemon(cfg: dict) -> Any:
    ...
    def _run() -> None:
        ...
        log.info("nba-bot starting (interval=%ds, repo=%s, "
        ...
        if live_cfg is not None:
            ...
            executor = SportLiveExecutor(
                ...
                bot_key=BOT_KEY, tournament="NBA", surface="Basketball",
                win_verb="winning")
        log.info("nba-bot upstream loaded; entering tick loop")
        while True:
            try:
                _one_tick(upstream, live_executor=executor,
                          paper_sim=paper_sim)
            except Exception:  # noqa: BLE001
                log.exception("nba-bot tick failed")
            time.sleep(interval)
