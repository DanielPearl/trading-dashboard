"""PCE bot — the cpi_bot package run with ``run.reference: pce``
(Cleveland Fed's PCE / Core PCE nowcast columns → KXPCECORE et al.).
...
log = logging.getLogger("dashboard.pce-bot")
...
def start_daemon(cfg: dict) -> Any:
    """Spawn the PCE background thread. Config::
        ...
        pce_trader:
          enabled: true
          repo_path: /root/cpi
          config_path: /root/cpi/config/config-pce.yaml
    """
    enabled = bool(cfg.get("enabled"))
    repo_path = cfg.get("repo_path", "/root/cpi")
    config_path = cfg.get("config_path",
                          "/root/cpi/config/config-pce.yaml")

    def _run() -> None:
        log.info("pce-bot starting (repo=%s)", repo_path)
        ...
        log.info("pce-bot upstream loaded; handing off to Bot.run()")
        bot.run()

    return _base.spawn_daemon("pce-bot", _run, enabled=enabled)
