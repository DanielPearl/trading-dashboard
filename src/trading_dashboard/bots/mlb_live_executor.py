"""MLB live executor — places REAL Kalshi orders on real money.
...
buy/sell mechanics are identical across bots. Adapted to KXMLBGAME
rows: one watchlist row per game, each side backed by its own binary
market (ticker_a / ticker_b), always bought as YES on the chosen side.

Safety model (same as world-cup):
  * dry_run defaults to TRUE — the executor never touches the order
    endpoint until the operator flips ``mlb_trader.live.dry_run``
    to false (or arms the Home-tab toggle on the live dashboard).
    Dry-run ticks log every order they WOULD place and accumulate
    paper positions in the live state file.
  ...
  * The Home-tab mlb toggle gates new orders every tick.
  * Extra MLB-specific gate: no row without a benchmark line ever
    reaches the executor (the exporter marks those ineligible), so
    every order is backed by a live Pinnacle/Betfair reference.
"""
...
log = logging.getLogger("dashboard.mlb-live-executor")

_HARD_MAX_CONTRACTS_PER_ORDER = 1
_HARD_MAX_OPEN_POSITIONS = 10
_HARD_MAX_ORDERS_PER_DAY = 15
_HARD_MIN_EDGE_PP = 0.05
_HARD_MAX_ENTRY_PRICE_CENTS = 80
_HARD_MIN_ENTRY_PRICE_CENTS = 15
_HARD_PRICE_DEVIATION_CENTS = 3
_HARD_MIN_PROFIT_LOCK_BID = 90


class MLBLiveExecutor:
    """Stateful executor wired into bots/mlb.py when the
    ``mlb_trader.live`` config block exists. Runs in the LIVE
    ...
    def __init__(self, cfg: dict, state_path: str) -> None:
        ...
        self.max_open = capped("max_open_positions", 6,
                               _HARD_MAX_OPEN_POSITIONS)
        self.max_orders_per_day = capped("max_orders_per_day", 10,
        ...
        self.max_entry_price_cents = capped(
            "max_entry_price_cents", 80, _HARD_MAX_ENTRY_PRICE_CENTS)
        ...
        self.prematch_buffer_minutes = float(
            cfg.get("prematch_buffer_minutes", 10) or 10)
        ...
        self._session = core.KalshiSession("mlb")

        log.info(
            "mlb-live-executor configured (dry_run=%s, max_open=%d, "
        ...
        if not self.dry_run:
            log.warning(
                "mlb-live-executor is in LIVE mode — real Kalshi "
                "orders will be placed. Flip the mlb Home-tab "
                "toggle OFF or set mlb_trader.live.dry_run back "
                "to true to stop.")
    ...
    def tick(self, watchlist_rows: list[dict],
        ...
        mlb Home-tab toggle (explicitly ON in bot_states_live) —
        ...
        if was_dry != self.dry_run:
            if self.dry_run:
                log.warning("mlb-live DISARMED (toggle off) — no "
                            "further real orders")
            else:
                log.warning("mlb-live ARMED via Home-tab toggle — real "
        ...
        if not self.dry_run:
            ...
            if voided:
                log.warning("mlb-live voided %d leftover dry-run "
    ...
    def _maybe_place(self, row: dict, state: dict) -> None:
        ...
        if self._daily.current() >= self.max_orders_per_day:
            log.info("mlb-live skip %s: max_orders_per_day (%d)",
        ...
        if not (self.min_entry_price_cents <= ask_cents
            ...
            log.info("mlb-live skip %s: ask %d¢ outside [%d, %d]",
        ...
        if float(model_p) <= 0.50:
            log.info("mlb-live skip %s: benchmark prob %.3f ≤ 0.50 — "
                     "refuse to buy a side the sharp line expects to "
                     "lose", ticker, float(model_p))
            return
        ...
        if kickoff:
            try:
                ...
                if mins <= self.prematch_buffer_minutes:
                    log.info("mlb-live skip %s: %.0f min to first pitch "
        ...
        if abs(current_ask - ask_cents) > self.price_deviation_cents:
            log.info("mlb-live skip %s: market moved %d¢ → %d¢",
        ...
        if balance is not None and balance < cost:
            log.warning("mlb-live skip %s: balance %d¢ < cost %d¢",
                        ticker, balance, cost)
            return
        ...
        position = {
            ...
            "tournament": row.get("tournament") or "MLB",
            "surface": row.get("surface") or "Baseball",
        ...
        log.info("mlb-live %s %s — BUY %d YES on %s (%s winning) at "
                 "%d¢ edge=%.3f",
    ...
    def _reconcile(self, state: dict, mkt_by_ticker: dict) -> None:
        ...
        for pos in state.get("open_positions", []):
            ...
            if result is not None:
                ...
                log.info("mlb-live SETTLED %s — %s, P&L %+.3f", ticker,
        ...
        for pos in state.get("open_positions", []):
            ...
            if self.dry_run:
                ...
                if held <= 0:
                    ...
                    log.warning(
                        "mlb-live %s already flat at Kalshi — recording "
            ...
            closed_now.append(closed)
            log.info("mlb-live PROFIT-LOCKED %s — sold @ %d¢, realized "
    ...
    def _load_state(self) -> dict:
        if self.state_path.exists():
            try:
                ...
                log.exception("mlb-live state load failed; fresh start")
        return {
