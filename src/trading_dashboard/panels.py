...
def _render_analytics_panel(out: List[str]) -> None:
    """Analytics alarms for the Home tab. The full "Model quality —
    calibration & CLV" table was retired 2026-09-14 (user: "this
    section is now redundant to what's in the model cards") — the
    bot cards carry P&L / ROI / win / Brier / edge / trades /
    drawdown / CLV plus the model-vs-market verdict now. What
    remains here is the one thing the cards can't show: the
    settlement-attribution alarm (user 2026-09-13: "no catch all
    label. all the bots should be known") — non-empty means History
    has rows no bot card claims; name the series so the fix is a
    one-line config change. Silent otherwise.
    """
...
BOT_FILTER_KEYS = {
    ...
    "rain", "temp", "weather", "cpi", "pce", "gdp", "unemployment-claims",
}
